...
function Kaigyou()
	...
	if DrawPointY * MOJI_SIZE + MOJI_SIZE > 480 then
		-- テンポラリグラフィックの作成
		TempGraph = dx.MakeGraph(640, 480)

		-- 画面の内容を丸々コピーする
		dx.GetDrawScreenGraph(0, 0, 640, 480, TempGraph)

		-- 一行分上に貼り付ける
		dx.DrawGraph(0, -MOJI_SIZE, TempGraph, false)

		-- 一番下の行の部分を黒で埋める
		dx.DrawBox(0, 480 - MOJI_SIZE, 640, 480, 0, true)
...
function dx.Update()
    ...
    elseif Moji == 'C' then	-- クリア文字
        ...
        -- １文字描画
        dx.DrawString(DrawPointX * MOJI_SIZE , DrawPointY * MOJI_SIZE,
            Moji, dx.GetColor(255 , 255 , 255))
